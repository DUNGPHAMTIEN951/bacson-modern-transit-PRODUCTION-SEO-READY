...
export async function getTrips(payload: { date: string; route: string }) {
  return request("GET_TRIPS", payload);
}

export async function getSeats(payload: { tripId: string }) {
  return request("GET_SEATS", payload);
}

export async function holdSeat(payload: { tripId: string; seat: string; phone?: string }) {
